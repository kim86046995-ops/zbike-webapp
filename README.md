...
- ✅ **해결**: `loanData` 객체에 `postcode`, `address`, `detail_address` 필드 추가
- ✅ **영향**: 이제 차용증 작성 시 고객 정보가 정상적으로 업데이트됨

**v7.50 (2026-02-23)** - 임시렌트 계약서에 주소 입력 필드 추가
- ✅ 임시렌트 고객 정보 섹션에 우편번호, 주소, 상세주소, 면허종류 필드 추가
- ✅ Daum 우편번호 API 연동
- ✅ `submitTempRentContract` 함수에서 주소 데이터 전송

**v7.43-7.49 (2026-02-23)** - 관리자 직접 저장 모드 추가
- ✅ 모든 계약서에 "SMS 링크 전송" / "즉시 완료 처리" 두 개 버튼 추가
- ✅ 관리자가 태블릿/스마트폰으로 고객 동의, 서명, 신분증을 직접 입력하여 즉시 완료 처리 가능
- ✅ `saveDirectlyByAdmin` 함수 추가 (계약서, 차용증, 업체 계약서 모두 지원)
- ✅ 안전한 요소 값 추출 (`getElementValue` 헬퍼 함수)
