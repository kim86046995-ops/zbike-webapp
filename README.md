...
- **프리뷰**: https://073f529f.zbike-webapp.pages.dev
...
## 📅 최종 업데이트
2026-02-03 - 오토바이 등록/관리 분리 완료
- 빠른 액세스에 독립 등록 페이지 추가 (/static/motorcycle-register)
- 총바이크 화면은 조회/수정/삭제 전용으로 변경
- 운전범위 드롭다운 선택 (전연령, 만19세이상, 만21세이상, 만24세이상, 만26세이상, 만30세이상, 만35세이상)
- 상태 필드 제거, 신규 등록 시 자동으로 '사용가능' 설정
- 업체 정보에 우편번호 추가 (데이터베이스 마이그레이션 포함)
- 업체 계약서 작성 시 우편번호 자동 입력
- 업체 상세정보에 우편번호, 주소, 상세주소 분리 표시
