# 오토바이 리스/렌트 전자계약 시스템

## 🌐 URLs
- **프로덕션**: https://zbike-webapp.pages.dev
- **최신 배포 (v8.14 - 계약서 상세 임대인 정보 정리)**: https://a3e66df8.zbike-webapp.pages.dev
- **대시보드**: https://a3e66df8.zbike-webapp.pages.dev/dashboard
- **GitHub**: https://github.com/kim86046995-ops/zbike-webapp
- **SMS 서버 (EC2)**: http://13.209.230.136:3001/sms

## 📅 최근 업데이트

**v8.14 (2026-02-28)** - 계약서 상세 임대인 정보 정리 🎉
- ✅ **임대인 정보 필드 정리**: 계약서 상세(모달)에서 "매니저" 전화번호 필드 제거
  - ❌ 이전: 상호, 업체계약번호, 대표자, 총괄매니저, **매니저** (2개 전화번호)
  - ✅ 수정: 상호, 업체계약번호, 대표자, **총괄매니저** (1개 전화번호만 표시)
- ✅ **정보 간소화**: 중복되는 매니저 전화번호 제거로 화면 깔끔하게 정리

**v8.13 (2026-02-28)** - 대시보드 자동 리다이렉트 🎉
- ✅ **자동 로그인 후 대시보드로 이동**: `/static/contracts.html` → `/dashboard`
- ✅ **로그인 성공 후 대시보드로 이동**: 로그인 후 운영 현황 대시보드로 자동 이동
- ✅ **대시보드 기능**: 총 바이크/사용중/휴차중/정비·폐지 통계, 바이크 목록 필터링
- ✅ **빠른 접근 메뉴**: 오토바이 등록, 계약서 작성, 차용증 작성, 각종 목록 바로가기

**v8.12 (2026-02-28)** - 자동 로그인 기능 추가 🎉
- ✅ **자동 로그인**: 한 번 로그인하면 로그아웃 전까지 자동 로그인 유지
  - localStorage에 세션 저장 (30일 유효)
  - 로그인 페이지 방문 시 자동으로 세션 검증
  - 유효한 세션이 있으면 자동으로 계약서 목록으로 리다이렉트
- ✅ **세션 검증 API 추가**: `GET /api/auth/validate`
  - 세션 유효성 실시간 확인
  - 만료된 세션 자동 제거
- ✅ **사용자 경험 개선**: 로그인 페이지를 계속 건너뛰어 더 빠른 접근

**v8.11 (2026-02-28)** - 업체 계약서 리다이렉트 수정 🎉
- ✅ **SMS 전송 후 리다이렉트**: 홈(`/`) → **계약서 목록**(`/static/contracts.html`)
- ✅ **관리자 직접 저장 후 리다이렉트**: 홈(`/`) → **계약서 목록**(`/static/contracts.html`)
- ✅ **이희준 고객 전화번호 삭제**: 고객 목록 및 모든 계약서에서 전화번호 제거

**v8.10 (2026-02-28)** - 업체 계약서 서명란 터치 개선 🎉
- ✅ **서명란 전체 터치 가능**: 캔버스 어디든 한 번만 터치하면 확대 모달 열림
  - ❌ 이전: 터치 범위가 좁아서 모달이 안 열리는 문제
  - ✅ 수정: **캔버스 전체 영역**을 터치 시 즉시 확대 모달 열림
- ✅ **간소화된 서명 패드 초기화**:
  - 메인 캔버스는 터치/클릭 시 즉시 모달만 열림
  - 모달 캔버스에서만 실제 서명 작성
  - 불필요한 이벤트 리스너 제거 (69줄 → 8줄)
- ✅ **안내 문구 개선**: "아래 박스를 터치하면 확대하여 서명할 수 있습니다"

**v8.9 (2026-02-28)** - 업체 계약서 신분증 자동화 🎉
- ✅ **서명 확대 모달**: 서명란 터치 시 확대 모달 제공 (이미 구현되어 있었음)
  - 서명란 클릭 → 전체화면 모달 열림
  - 큰 캔버스에서 편리하게 서명
  - 저장 버튼 클릭 → 메인 서명란에 자동 복사
- ✅ **신분증 자동 업로드**: 업체는 업체 목록에 등록된 신분증 자동 사용
  - ❌ 이전: 고객이 직접 신분증 업로드 필요 → 업로드 안 하면 전송 실패
  - ✅ 수정: **업체 목록 신분증 자동 사용** → 즉시 전송 가능
  - 백엔드: `companies` 테이블에서 `id_card_photo` 자동 조회
  - 프론트엔드: 신분증 체크 로직 제거
- ✅ **사용자 경험 개선**:
  - 약관 동의 체크 → 서명 → 제출 (신분증 업로드 단계 제거)
  - "신분증을 촬영하거나 업로드해주세요" 에러 메시지 제거

**v8.8 (2026-02-28)** - 업체 계약서 계약번호 표시 🎉
- ✅ **업체 정보 라벨 변경**: "사업자등록번호" → "계약번호"
  - ❌ 이전: 사업자등록번호 (`business_number`)
  - ✅ 수정: **계약번호** (`contract_number`) - 파란색 강조
- ✅ **표시 형식**: 계약번호는 자동 생성된 고유 번호 표시
  - 예시: BC-20260228-0001 (Business Contract)

**v8.7 (2026-02-28)** - 업체 계약서 보험정보 개선 🎉
- ✅ **리다이렉트 수정**: 관리자 직접 저장 후 홈페이지(`/`)로 이동 (기존: `/static/contracts.html`)
- ✅ **보험 정보 추가**: SMS 링크 계약서에 보험사 + 운전연령 표시
  - 보험사 (`insurance_company`)
  - 보험 시작일 (`insurance_start_date`)
  - 보험 종료일 (`insurance_end_date`)
  - **운전가능연령** (`driving_range`) ← 새로 추가 (주황색 강조)
- ✅ **백엔드 수정**: `/api/business-contracts/:id/sign` API에 오토바이 정보 추가
  - `m.driving_range` (운전연령)
  - `m.insurance_company` (보험사)
  - `m.insurance_start_date` (보험 시작일)
  - `m.insurance_end_date` (보험 종료일)

**v8.6 (2026-02-28)** - 업체 계약서 계약 종류 추가 🎉
- ✅ **업체 정보**: 업종, 업태 삭제
  - ❌ 삭제: 업종 (`business_type`)
  - ❌ 삭제: 업태 (`business_category`)
- ✅ **계약 조건**: 계약 종류 추가 (최상단 표시)
  - ✅ 추가: **계약 종류** (`business_contract_type`)
  - 리스 (`lease`) → "리스"
  - 렌트 (`rent`) → "렌트"
  - 임시렌트 (`temp_rent`) → "임시렌트"
- ✅ **표시 순서**: 계약 종류 → 계약 시작일 → 계약 종료일 → 일 차감액 → 보증금

**v8.5 (2026-02-28)** - 업체 계약서 정보 표시 개선 🎉
- ✅ **"임차인 정보" → "업체 정보"** 변경
- ✅ **업체 정보 표시**: 업체목록에서 모든 정보 가져와서 표시
  - 업체명, 사업자등록번호
  - 대표자, 대표자 주민등록번호, 대표자 연락처, 대표자 주소
  - 업종, 업태
  - 업체 연락처, 업체 주소
- ✅ **계약 내용**: 관리자가 입력한 그대로 표시
  - 계약 시작일, 계약 종료일
  - 일 차감액, 보증금
  - ❌ **운전연령 제거** (계약 내용에서 삭제)
- ✅ **오토바이 정보**: 기본 정보 + 차량 제원 + 보험 정보만 표시
  - 차량명, 차량번호, 차대번호
  - 연식, 주행거리
  - 보험 시작일, 보험 종료일

**v8.4 (2026-02-28)** - 업체 계약서 리다이렉트 수정 🎉
- ✅ **문제 해결**: SMS 전송 후 404 에러 발생 (`/static/contracts-list.html` 파일 없음)
- ✅ **수정 후**: SMS 전송 후 홈페이지(`/`)로 리다이렉트
- ✅ **사용자 경험**: 3초 후 자동으로 홈페이지로 이동

**v8.3 (2026-02-28)** - 업체 계약서 약관 동의 수정 🎉
- ✅ **문제 해결**: 관리자가 SMS 전송 시 약관 동의 체크 요구 제거
- ✅ **약관 동의는 고객이 직접**: 업체 대표자가 SMS 링크 클릭 후 약관 동의 체크
- ✅ **관리자 직접 저장**: 관리자가 현장에서 직접 동의·서명 받을 때만 약관 동의 체크
- ✅ **명확한 구분**:
  - **SMS 전송 버튼**: 약관 동의 체크 **불필요** (고객이 나중에 동의)
  - **즉시 완료 버튼**: 약관 동의 체크 **필수** (관리자가 현장에서 동의 받음)

**v8.2 (2026-02-28)** - 업체 계약서 SMS 통합 완료 🎉
- ✅ **업체 계약서 SMS 개선**: 차용증과 동일한 워크플로우 적용
  - **관리자 작성**: 업체 정보 + 오토바이 선택 + 계약 정보 입력 → SMS 발송
  - **고객 확인**: 문자 링크 클릭 → 계약서 원본 확인 (회사 정보, 업체 정보, 오토바이 정보, 계약 조건, 약관)
  - **고객 서명**: 약관 동의 → 전자서명 → 신분증 업로드 → 제출
  - **자동 저장**: 제출 완료 시 계약서 상태 'active'로 변경 및 목록에 자동 저장
- ✅ **SMS 메시지 문구**: `contract_type`에 따라 자동 변경
  - `business` → **"[Z-BIKE 업체 계약서]"**
  - `loan` → **"[Z-BIKE 차용증 계약서]"**
  - 기타 → **"[Z-BIKE 전자계약서]"**
- ✅ **백엔드 수정**: `/api/business-contracts/:id/sign` GET/PUT 모두 `company_settings` 포함
- ✅ **인증 없이 접근**: 고객이 로그인 없이 SMS 링크로 계약서 확인 가능
- ✅ **신분증 업로드 필수**: 업체 대표자가 직접 신분증을 업로드 (업체에 등록된 신분증 아님)

**v8.1 (2026-02-28)** - 차용증 SMS 링크 계약서 원본 정보 표시 완료 🎉
- ✅ **차용증 SMS 링크 개선**: 고객이 받은 문자 링크에 모든 계약 정보 표시
  - **채권자 정보**: 회사명, 사업자등록번호, 대표자, 주소, 연락처
  - **차용인 정보**: 성명, 연락처, 주민등록번호, 주소
  - **대여금 정보**: 대여금액, 일차감액, 대여일, 상환일, 대여기간
  - **특약사항**: 추가 조건 및 특약사항
  - **약관**: 전문적인 금전소비대차 약관 (제1조~제12조)
  - **동의 체크**: 필수 약관 동의 체크박스
  - **전자서명**: 터치/마우스 서명 지원
  - **신분증 업로드**: 카메라 촬영 또는 파일 업로드
- ✅ **백엔드 수정**: `/api/loan-contracts/:id/sign` API에 `company_settings` 포함
- ✅ **인증 없이 접근**: 고객이 로그인 없이 SMS 링크로 계약서 확인 가능
- ✅ **관리자 흐름**: 고객정보 + 대출금액만 입력 → SMS 발송
- ✅ **고객 흐름**: 문자링크 클릭 → 약관동의 → 서명 → 신분증 업로드 → 제출
- ✅ **계약서 목록**: 서명 전 [서명대기] (파란색), 서명 후 [진행중] (초록색)

**v8.0 (2026-02-27)** - 알리고 SMS 연동 완료 🎉
- ✅ **EC2 SMS 서버 구축**: Node.js Express 서버 (http://13.209.230.136:3001/sms)
- ✅ **알리고 SMS API 연동**: 고정 IP (13.209.230.136) 기반 인증
- ✅ **PM2 프로세스 관리**: 자동 재시작 및 서버 부팅 시 자동 실행
- ✅ **Cloudflare Pages Secrets**: SMS 환경변수 안전한 관리
- ✅ **SMS 잔액**: 11,855건
- ✅ **발신번호**: 01086046995
- ✅ **API 정보**: User ID (sangchyn11), API Key (pe2pyx6zg5aqszgwr4mcwa59vrcbyrx9)

**v7.51 (2026-02-23)** - 차용증 작성 시 고객 주소 정보 업데이트 수정
- ✅ **문제**: 차용증 작성 시 고객 테이블에 주소 정보(`postcode`, `address`, `detail_address`)가 업데이트되지 않는 문제
- ✅ **원인**: 프론트엔드에서 `borrower_address` (합쳐진 주소)만 전송하고, 개별 주소 필드를 전송하지 않음
- ✅ **해결**: `loanData` 객체에 `postcode`, `address`, `detail_address` 필드 추가
- ✅ **영향**: 이제 차용증 작성 시 고객 정보가 정상적으로 업데이트됨

**v7.50 (2026-02-23)** - 임시렌트 계약서에 주소 입력 필드 추가
- ✅ 임시렌트 고객 정보 섹션에 우편번호, 주소, 상세주소, 면허종류 필드 추가
- ✅ Daum 우편번호 API 연동
- ✅ `submitTempRentContract` 함수에서 주소 데이터 전송

**v7.43-7.49 (2026-02-23)** - 관리자 직접 저장 모드 추가
- ✅ 모든 계약서에 "SMS 링크 전송" / "즉시 완료 처리" 두 개 버튼 추가
- ✅ 관리자가 태블릿/스마트폰으로 고객 동의, 서명, 신분증을 직접 입력하여 즉시 완료 처리 가능
- ✅ `saveDirectlyByAdmin` 함수 추가 (계약서, 차용증, 업체 계약서 모두 지원)
- ✅ 안전한 요소 값 추출 (`getElementValue` 헬퍼 함수)

## 프로젝트 개요
- **이름**: 오토바이 리스/렌트 전자계약 시스템
- **목적**: 오토바이 리스/렌트 업체를 위한 전자 계약서 작성 및 관리 시스템
- **주요 기능**: 오토바이 관리, 전자 계약서 작성, 전자서명, 계약 내역 조회

## 🌐 URLs
- **프로덕션**: https://zbike-webapp.pages.dev
- **최신 배포 (v7.5 - 업체 상세 정보 표시 수정)**: https://d7e42cef.zbike-webapp.pages.dev
- **GitHub**: https://github.com/kim86046995-ops/zbike-webapp
- **업체 목록**: https://d7e42cef.zbike-webapp.pages.dev/static/companies-list.html
- **업체 등록**: https://d7e42cef.zbike-webapp.pages.dev/static/company-register.html
- **업체 계약서**: https://d7e42cef.zbike-webapp.pages.dev/static/business-contract-new.html

## ✨ 주요 기능

### 🔐 인증 및 보안
- ✅ **로그인 시스템**
  - 세션 기반 인증 (24시간 유효)
  - 자동 세션 만료 처리
  - 로그인 페이지로 자동 리디렉션
  - 로그아웃 기능
- ✅ **계정 찾기 기능**
  - 아이디 찾기 (이름 + 전화번호)
  - 비밀번호 재설정 (아이디 + 이름 + 전화번호)
  - 6자리 인증번호 생성 (30분 유효)
  - 본인 확인 후 비밀번호 재설정
- ✅ **관리자 페이지 보호**
  - 모든 관리 기능 인증 필요
  - API 레벨 인증 미들웨어
  - 401 에러 자동 처리
- ✅ **관리자 계정**
  - 아이디: sangchun11
  - 비밀번호: a2636991!@#

### 1. 오토바이 관리
- ✅ **오토바이 등록 (독립 페이지)**
  - 빠른 액세스에서 전용 등록 페이지로 이동
  - 차량번호, 차량이름, 차대번호
  - 키로수, 연식
  - 보험사, 보험기간 (시작일~종료일)
  - 운전범위 (드롭다운: 전연령, 만19세이상, 만21세이상, 만24세이상, 만26세이상, 만30세이상, 만35세이상)
  - 명의자
  - 보험료, 차량금액
  - 상태 자동 설정 (신규 등록 시 자동으로 '사용가능')
  - 등록 완료 후 목록 페이지로 자동 이동
- ✅ **오토바이 목록 조회 및 관리**
  - 조회/수정/삭제 전용 화면
  - 검색 및 필터링
  - 계약 이력 조회
- ✅ 오토바이 정보 수정/삭제 (인증 필요)
- ✅ 실시간 상태 관리
- ✅ **보험 기간 진행률 시각화** (프로그레스 바)
  - 날짜가 지날수록 자동으로 줄어드는 프로그레스 바
  - 만료 임박 시 경고 표시 (30일 이내)
  - 만료된 보험 빨간색 강조 표시
  - 남은 일수 실시간 계산 및 표시
  - 시각적 색상 구분 (안전:파란색, 경고:주황색, 위험:빨간색)
- ✅ **번호판 클릭으로 계약 이력 조회**
  - 해당 오토바이의 모든 계약 이력 모달로 표시
  - 계약 상태별 구분 (진행중/완료/취소)
  - 계약 타입별 구분 (리스/렌트)
  - 고객 정보 및 계약 상세 내용 표시
  - 계약번호, 계약기간, 납부금액 등 상세 정보

### 2. 계약서 작성
- ✅ 계약 타입 선택 (리스/렌트)
- ✅ 사용 가능한 오토바이 선택
- ✅ 고객 정보 입력
  - 이름, 주민번호
  - 전화번호, 주소
  - 면허종류 (2종 소형, 2종 보통, 1종 보통, 1종 대형)
- ✅ 계약 상세 정보
  - 계약 기간 (시작일~종료일)
  - 월 납부금액
  - 보증금
  - 특약사항
  - 미리보기 및 재촬영
  - Base64 형식으로 저장
- ✅ 전자서명 기능 (캔버스 기반)
  - 마우스/터치 서명 지원
  - 서명 이미지 저장 (base64)
- ✅ 자동 계약번호 생성 (YYYYMMDD-XXXX 형식)

### 3. 계약서 관리
- ✅ 계약서 목록 조회
- ✅ 필터링 (계약 타입, 상태)
- ✅ 검색 (계약번호, 고객명, 번호판)
- ✅ 계약서 상세 보기
  - 계약 정보
  - 고객 정보
  - 오토바이 상세 정보
  - 계약 상세 내용
  - 서명 이미지
  - 신분증 사진
- ✅ 계약 상태 관리 (진행중/완료/취소)
- ✅ **계약 해지 시 종료일 자동 설정** 🆕 (해지일/완료일이 자동으로 오늘 날짜로 기록)
- ✅ **완벽한 계약 이력 관리 시스템** 🆕
  - 🔒 **강력한 보호**: 수정/삭제 절대 불가 (트리거 기반 보호)
  - 📝 **누적 방식**: 모든 액션은 새로운 행으로 추가 (덮어쓰기 금지)
  - 🎯 **계약번호 포함**: 상세 이력 추적 및 식별
  - 🔄 **자동 대체 처리**: 덮어쓰기 시 이전 계약 자동 해지 및 이력 보존
  - 📊 **컴팩트 UI**: 50% 공간 절약, 최대 높이 제한 + 스크롤
  - ⚖️ **법적 증거**: 완벽한 감사 추적(Audit Trail)

### 4. 계약자 관리 🆕
- ✅ **계약자 정보 등록**
  - 이름, 주민등록번호 (자동 하이픈)
  - 전화번호 (자동 하이픈)
  - 우편번호 검색 (Daum API)
  - 주소 자동 입력 (도로명/지번)
  - 상세주소 입력
- ✅ **계약자 목록 페이지**
  - 카드 기반 레이아웃
  - 검색 기능 (이름/전화번호/주소)
  - 상세 정보 모달
  - 반응형 디자인 (1열/2열/3열)
  - 주민등록번호 마스킹 (123456-*******)
- ✅ **자동 워크플로우**
  - 저장 후 자동으로 계약자 목록으로 이동
  - 입력란 자동 초기화
  - 빠른 계속 입력 가능

### 5. PDF 다운로드 기능
  - 모든 계약서 정보 포함 (약관, 동의, 서명, 신분증)
  - A4 최적화 레이아웃
  - File System Access API 지원 (Chrome/Edge)
  - 다른 이름으로 저장 기능
  - 직접 다운로드 (Blob 방식)
  - 목록에서 빠른 다운로드 지원

### 4. 업체 정보 관리 🆕
- ✅ **업체 정보 등록**
  - 업체명
  - **업체 코드 자동 생성** (형식: AUTO-YYYYMMDD-XXX)
    - 페이지 로드 시 즉시 생성 및 표시
    - 등록 날짜 포함
    - 랜덤 3자리 숫자로 중복 방지
    - 변경 불가능
  - 대표자명, 전화번호 (자동 하이픈)
  - 주민등록번호 (자동 하이픈)
  - 우편번호 검색 (Daum API)
  - 대표자 주소 자동 입력
  - 상세주소 입력
  - 신분증 사진 업로드 및 미리보기
- ✅ **업체 목록 페이지**
  - 카드 기반 레이아웃
  - 검색 기능 (업체명/업체 코드/대표자명/전화번호)
  - 상세 정보 모달 (업체 정보, 대표자 정보, 주소, 신분증)
  - 반응형 디자인
  - 등록일 표시
  - 업체 코드 자동 표시
- ✅ **업체 삭제 기능** (Soft Delete)
  - 각 업체 카드에 삭제 버튼
  - 삭제 확인 대화상자
  - 삭제된 업체는 status='inactive'로 변경
  - 목록에서 자동으로 제거
  - 삭제 후 자동 목록 새로고침
  - 삭제된 업체 코드 재사용 가능 (중복 체크 시 active만 확인)
- ✅ **자동 워크플로우**
  - 등록 후 자동으로 업체 목록으로 이동
  - 빠른 액세스에서 업체 목록 직접 접근 가능
  - 업체 등록 시 active 업체만 중복 검증
- ✅ **업체 계약서 작성**
  - 업체 코드 필드 자동 입력
  - 업체 선택 시 코드 자동 표시
  - 읽기 전용 필드 (수정 불가)

### 5. 차용증 관리 🆕
- ✅ 차용증 작성
  - 차용인 정보 (이름, 주민번호, 전화번호, 주소)
  - 대여인 정보 (이름, 주민번호, 전화번호, 주소)
  - 차용 금액 및 이자율
  - 차용일 및 상환일
  - 상환 방법 (일시불/분할)
  - 담보 정보
  - 특약사항
- ✅ **차용증 약관 (전문적인 법적 조항)**
  - 제1조: 차용금액 및 기간
  - 제2조: 상환방법
  - 제3조: 담보 및 보증
  - 제4조: 기한의 이익 상실
  - 제5조: 연체이자 및 지연배상금 (연 15%)
  - 제6조: 비용 부담
  - 제7조: 계약의 해석 및 합의
  - 제8조: 관할 법원
- ✅ 이중 전자서명 (차용인 + 대여인)
- ✅ **신분증 촬영 기능**
- ✅ 차용증 목록 및 상세 보기
- ✅ 상태 관리 (진행중/완료/연체/취소)
- ✅ **PDF 다운로드 기능** 🆕
  - 전체 약관, 동의, 서명, 신분증 포함
  - A4 최적화 레이아웃
  - 다른 이름으로 저장
  - 목록에서 빠른 다운로드

## 📊 데이터 아키텍처

### 저장소 (Storage)

#### Cloudflare D1 (SQLite) - 구조화된 데이터
텍스트 데이터, 관계형 데이터 저장 (최대 1MB/컬럼)

#### Cloudflare R2 Storage - 파일 저장소 🆕
- **신분증 이미지** 저장 (D1 1MB 제한 회피)
- **버킷**: `zbike-id-cards`
- **바인딩**: `R2_ID_CARDS`
- **파일 경로**: `id-cards/{timestamp}-{random}.jpg`
- **공개 URL**: R2 public domain을 통한 접근
- DB에는 R2 URL만 저장 (텍스트)

### 데이터베이스: Cloudflare D1 (SQLite)

#### 테이블 구조

**1. motorcycles (오토바이)**
- id, plate_number (번호판), vehicle_name (차량명)
- chassis_number (차대번호), mileage (키로수), model_year (연식)
- insurance_company (보험사), insurance_start_date, insurance_end_date
- driving_range (운전범위), owner_name (명의자)
- insurance_fee (보험료), vehicle_price (차량가격)
- **daily_rental_fee (일대여료)**
- **usage_notes (사용 현황 메모)**
- status (상태: available/rented/maintenance)

**2. customers (고객)**
- id, name (이름), resident_number (주민번호)
- phone (전화번호), address (주소), license_type (면허종류)

**3. contracts (계약서)**
- id, contract_type (리스/렌트), contract_number (계약번호)
- motorcycle_id, customer_id
- start_date, end_date (계약기간)
- **daily_rental_fee (일대여료)**, deposit (보증금)
- special_terms (특약사항)
- signature_data (서명 base64)
- **id_card_photo (신분증 사진 base64)**
- status (상태: active/completed/cancelled)

**4. loan_contracts (차용증) 🆕**
- id, loan_number (차용증 번호)
- borrower_name, borrower_resident_number, borrower_phone, borrower_address (차용인)
- lender_name, lender_resident_number, lender_phone, lender_address (대여인)
- loan_amount (차용금액), loan_date (차용일), repayment_date (상환일)
- interest_rate (이자율), repayment_method (상환방법)
- collateral (담보), special_terms (특약사항)
- borrower_signature, lender_signature (이중 서명)
- borrower_id_card_photo (차용인 신분증)
- status (상태: active/completed/overdue/cancelled)

### API 엔드포인트

#### 인증 API (Authentication)
- `POST /api/auth/login` - 로그인 (username, password)
- `POST /api/auth/logout` - 로그아웃 (세션 ID 필요)
- `GET /api/auth/check` - 세션 유효성 확인
- `POST /api/auth/find-username` - 아이디 찾기 (name, phone)
- `POST /api/auth/request-reset` - 비밀번호 재설정 토큰 생성 (username, name, phone)
- `POST /api/auth/reset-password` - 비밀번호 재설정 (token, new_password)

#### 오토바이 API (인증 필요)
- `GET /api/motorcycles` - 목록 조회 (상태 필터 가능)
- `GET /api/motorcycles/:id` - 상세 조회
- `GET /api/motorcycles/:id/contracts` - **오토바이별 계약 이력 조회**
- `POST /api/motorcycles` - 등록 ⚠️ **인증 필요**
- `PUT /api/motorcycles/:id` - 수정 ⚠️ **인증 필요**
- `DELETE /api/motorcycles/:id` - 삭제 ⚠️ **인증 필요**

#### 고객 API
- `GET /api/customers` - 목록 조회
- `GET /api/customers/:id` - 상세 조회
- `POST /api/customers` - 등록
- `PUT /api/customers/:id` - 수정

#### 계약서 API (인증 필요)
- `GET /api/contracts` - 목록 조회 (JOIN으로 관련 정보 포함)
- `GET /api/contracts/:id` - 상세 조회
- `POST /api/contracts` - 생성 ⚠️ **인증 필요**
- `PATCH /api/contracts/:id/status` - 상태 변경 ⚠️ **인증 필요**


#### 차용증 API 🆕
- `GET /api/loan-contracts` - 목록 조회
- `GET /api/loan-contracts/:id` - 상세 조회
- `POST /api/loan-contracts` - 생성
- `PATCH /api/loan-contracts/:id/status` - 상태 변경

#### 업체 관리 API 🆕
- `POST /api/upload/id-card` - 신분증 이미지 업로드 (R2 Storage)
  - Request: `{ image: "data:image/jpeg;base64,..." }`
  - Response: `{ success: true, url: "https://...", fileName: "..." }`
- `POST /api/companies` - 업체 등록 (id_card_photo는 R2 URL)
- `GET /api/companies` - 업체 목록 조회
- `GET /api/companies/search?q=<query>` - 업체 검색

## 📱 사용 가이드

### 0. 로그인 (필수)
1. 메인 페이지 또는 관리 페이지 접속
2. 자동으로 로그인 페이지로 이동
3. 관리자 계정으로 로그인
   - **아이디**: sangchun11
   - **비밀번호**: a2636991!@#
4. 로그인 성공 시 이전 페이지로 자동 이동

### 0-1. 아이디/비밀번호 찾기
1. 로그인 페이지에서 "아이디 또는 비밀번호를 잊으셨나요?" 클릭
2. **아이디 찾기**
   - 이름과 전화번호 입력
   - 일치하는 아이디 표시
3. **비밀번호 찾기**
   - 1단계: 아이디, 이름, 전화번호로 본인 확인
   - 2단계: 6자리 인증번호 확인 (30분 유효)
   - 3단계: 새 비밀번호 설정
   - 완료 후 로그인 페이지로 이동

### 1. 오토바이 등록
1. 로그인 후 메인 페이지에서 **빠른 액세스 > "오토바이 등록"** 클릭
2. 전용 등록 페이지에서 모든 필수 정보 입력
   - 차량번호, 차량이름, 차대번호
   - 키로수, 연식
   - 보험 정보 (보험사, 시작일, 종료일, 운전범위)
   - 명의자, 보험료, 차량금액
   - 사용 현황 메모 (선택)
3. "등록하기" 버튼 클릭
4. 등록 완료 후 자동으로 오토바이 목록 페이지로 이동

### 1-1. 오토바이 관리 (조회/수정/삭제)
1. 메인 페이지에서 "총바이크: X대" 클릭
2. 오토바이 목록 화면에서:
   - 검색 (번호판/계약자/명의자)
   - 상태 필터링 (전체/사용가능/렌트중/정비중/폐지)
   - 정렬 (최신순/번호판순/보험만료순 등)
3. 각 오토바이 카드에서:
   - **번호판 클릭**: 계약 이력 조회
   - **수정 버튼**: 정보 수정
   - **삭제 버튼**: 오토바이 삭제
   - **해지 버튼**: 모든 진행중인 계약 해지

### 2. 보험 기간 확인
1. 오토바이 관리 페이지에서 각 오토바이 카드 확인
2. 보험 기간 프로그레스 바 색상 확인
   - **파란색**: 안전 (30일 이상 남음)
   - **주황색**: 경고 (30일 미만 남음)
   - **빨간색**: 위험 (만료됨)
3. 남은 일수 실시간 표시 확인

### 3. 계약 이력 조회
1. 오토바이 관리 페이지에서 **번호판 클릭**
2. 해당 오토바이의 전체 계약 이력 모달 표시
3. 계약 상태, 고객 정보, 계약 상세 확인

### 4. 계약서 작성
1. 로그인 후 메인 페이지에서 "계약서 작성" 클릭
2. 계약 타입 선택 (리스/렌트)
3. 사용 가능한 오토바이 선택
4. 고객 정보 입력
5. 계약 상세 정보 입력 (기간, 금액 등)
6. 서명 박스에 고객 서명 받기
7. "계약서 생성" 버튼 클릭

### 5. 계약서 조회
1. 로그인 후 메인 페이지에서 "계약서 목록" 클릭
2. 필터/검색으로 원하는 계약서 찾기
3. "상세보기" 버튼으로 전체 내용 확인

### 6. 로그아웃
1. 메인 페이지 우측 상단 "로그아웃" 버튼 클릭
2. 확인 후 로그인 페이지로 이동


## 🚀 배포 정보

### 플랫폼
- **배포 플랫폼**: Cloudflare Pages
- **데이터베이스**: Cloudflare D1 (SQLite)
- **상태**: ✅ 개발 서버 실행 중

### 기술 스택
- **Backend**: Hono (v4) + TypeScript
- **Frontend**: HTML5 + TailwindCSS + JavaScript (Vanilla)
- **Database**: Cloudflare D1 (SQLite)
- **Icons**: FontAwesome 6
- **HTTP Client**: Axios
- **Deployment**: Cloudflare Pages

### 로컬 개발
```bash
# 데이터베이스 마이그레이션
npm run db:migrate:local

# 빌드
npm run build

# 개발 서버 시작 (PM2)
pm2 start ecosystem.config.cjs

# 서버 상태 확인
pm2 list

# 로그 확인
pm2 logs --nostream
```

### 프로덕션 배포
```bash
# 프로덕션 데이터베이스 마이그레이션
npm run db:migrate:prod

# Cloudflare Pages 배포
npm run deploy:prod
```

## 📝 완료된 기능

### 🔐 인증 및 보안
- ✅ **세션 기반 로그인 시스템**
  - 24시간 유효 세션
  - 자동 로그인 확인
  - 로그인 페이지 자동 리디렉션
- ✅ **계정 찾기 및 복구**
  - 아이디 찾기 (이름 + 전화번호)
  - 비밀번호 재설정 (6자리 인증번호)
  - 30분 유효 인증 토큰
  - 본인 확인 3단계 프로세스
- ✅ **관리자 페이지 보호**
  - API 레벨 인증 미들웨어
  - 401 에러 자동 처리
  - 로그아웃 기능
- ✅ **관리자 계정 관리**
  - 맞춤 관리자 계정 (sangchun11)
  - 데이터베이스 기반 인증
  - 이메일/전화번호 등록

### 🏍️ 오토바이 관리
- ✅ 오토바이 정보 등록 및 관리 (인증 필요)
- ✅ **보험 기간 시각화**
  - 자동으로 줄어드는 프로그레스 바
  - 색상별 상태 표시 (파란색/주황색/빨간색)
  - 남은 일수 실시간 계산
  - 만료 임박/만료 경고
- ✅ **번호판 클릭으로 계약 이력 조회**
  - 해당 오토바이의 모든 계약 이력 모달
  - 계약 상태별 구분 (진행중/완료/취소)
  - 고객 정보 및 상세 내용 표시

### 📄 계약서 시스템
- ✅ 리스/렌트 계약서 작성 (인증 필요)
- ✅ 전자서명 (캔버스 기반)
- ✅ 계약서 목록 및 검색
- ✅ 계약서 상세 보기
- ✅ 자동 계약번호 생성
- ✅ 계약 상태 관리 (인증 필요)

### 👥 고객 관리
- ✅ 고객 정보 등록
- ✅ 고객 정보 조회 및 수정
  - 자동 리다이렉트 (이미 로그인 시)
  - 실시간 로그인 상태 표시
- ✅ **계약서 공유 시스템**
  - SMS/카카오톡으로 계약서 링크 전송
  - 72시간 유효한 보안 토큰
  - 고객이 링크로 접속하여 서명
  - 서명 완료 시 자동 계약 생성

### 6. SMS 문자 전송 🎉 완료!
- ✅ **알리고 SMS API 연동 완료**
  - EC2 고정 IP 기반 인증 (13.209.230.136)
  - Node.js Express 서버 (http://13.209.230.136:3001/sms)
  - PM2 프로세스 관리 (자동 재시작)
  - Lightsail 방화벽 포트 3001 오픈
- ✅ **Cloudflare Pages 연동 완료**
  - Environment Secrets로 안전한 관리
  - SMS_ENABLED=true (활성화)
  - SMS_TEST_MODE=false (실제 전송)
  - SMS_AWS_LAMBDA_URL=http://13.209.230.136:3001/sms
- ✅ **SMS 잔액**: 11,855건
- ✅ **발신번호**: 01086046995
- ✅ **계약서 공유 링크 문자 발송 지원**

### 7. 계약서 서명 기능 🎨
- ✅ **서명 확대 모달**
  - 서명 영역 터치/클릭 시 전체 화면으로 확대
  - 모바일: 100% 전체 화면
  - 데스크톱: 90% 크기 (중앙 정렬)
  - 마우스/터치 모두 지원
  - 서명 동기화 (메인 ↔ 모달)
- ✅ **신분증 사진 비율 유지**
  - 원본 비율 100% 유지
  - 자동 리사이징 (최대 1920x1080)
  - 파일 크기 최적화 (95% 감소)
  - JPEG 압축 (80% 품질)
  - 카메라 촬영/파일 업로드 모두 지원

### 8. 통합 계약 해지 시스템 🔄
- ✅ **오토바이 관리에서 해지**
  - [해지] 버튼 클릭 시 모든 관련 계약 자동 처리
  - 진행중인 모든 계약을 'cancelled' 상태로 변경
  - 오토바이 상태를 '휴차중'으로 변경
  - 계약 정보 및 보험 정보 자동 초기화
  - 해지된 계약 수 표시
- ✅ **계약서 목록**
  - 해지된 계약도 목록에 표시 (기록 보존)
  - '취소' 배지로 명확히 구분
  - 상세보기 및 PDF 다운로드 가능
- ✅ **대시보드 (개인계약 보드)**
  - 진행중인 계약만 표시 (status = 'active')
  - 활성 계약 수 정확히 집계
  - 월 예상 수익 정확히 계산
- ✅ **데이터 일관성**
  - 한 곳에서 해지 → 모든 곳에 자동 반영
  - 계약서 목록과 대시보드 동기화
  - 감사 추적을 위한 완벽한 기록 보존

## ⚙️ SMS 설정 가이드 (알리고)

### ✅ 알리고 SMS 연동 완료 (2026-02-27)

**시스템 아키텍처:**
```
Cloudflare Pages → AWS EC2 (13.209.230.136:3001) → 알리고 SMS API
```

**EC2 SMS 서버 정보:**
- **서버 URL**: http://13.209.230.136:3001/sms
- **고정 IP**: 13.209.230.136 (알리고 화이트리스트 등록)
- **프로세스 관리**: PM2 (자동 재시작, 부팅 시 자동 실행)
- **방화벽**: Lightsail 포트 3001 오픈
- **서버 파일**: `/home/bitnami/lambda/server.js`

**알리고 API 설정:**
- **User ID**: sangchyn11
- **API Key**: pe2pyx6zg5aqszgwr4mcwa59vrcbyrx9
- **발신번호**: 01086046995
- **SMS 잔액**: 11,855건
- **등록일**: 2026-02-25

**Cloudflare Pages Secrets:**
```bash
SMS_ENABLED=true
SMS_TEST_MODE=false
SMS_AWS_LAMBDA_URL=http://13.209.230.136:3001/sms
ADMIN_PHONE=01086046995
```

### SMS 전송 플로우
1. 관리자가 계약서 작성 완료
2. "고객에게 전송" 버튼 클릭
3. Cloudflare Pages → EC2 SMS 서버 → 알리고 API
4. 고객 휴대폰 번호로 링크 전송
5. 고객이 링크 클릭하여 계약서 검토
6. 전자서명 및 신분증 업로드
7. 제출 시 자동으로 계약서 생성

### EC2 서버 관리 명령어
```bash
# PM2 서버 상태 확인
pm2 list

# 로그 확인
pm2 logs zbike-sms --nostream

# 서버 재시작
pm2 restart zbike-sms

# 서버 중지
pm2 stop zbike-sms

# 서버 시작
pm2 start zbike-sms

# SMS 잔액 확인
curl -X POST http://13.209.230.136:3001/sms \
  -H "Content-Type: application/json" \
  -d '{"action":"balance"}'
```

## 🔜 향후 개선 사항
- ✅ **PDF 다운로드 기능** (html2pdf.js 라이브러리 사용 - 완료!)
- ✅ **계약서 인쇄 최적화** (A4 최적화 완료)
- ⏳ 카카오 알림톡 연동
- ⏳ 계약 만료 알림
- ⏳ 통계 대시보드
- ⏳ 권한 관리 (일반 관리자/최고 관리자)
- ⏳ 계약서 템플릿 커스터마이징
- ⏳ 모바일 앱 버전

## 📄 라이선스
MIT License

## 👤 작성자
오토바이 리스/렌트 업체 관리자

## 📅 최종 업데이트
2026-02-27 v8.0 - 알리고 SMS 연동 완료 🎉
- ✅ EC2 Node.js SMS 서버 구축 (http://13.209.230.136:3001/sms)
- ✅ 알리고 SMS API 연동 (고정 IP 13.209.230.136)
- ✅ PM2 프로세스 관리 (자동 재시작, 부팅 시 자동 실행)
- ✅ Cloudflare Pages Secrets 설정 완료
- ✅ SMS 잔액 11,855건, 발신번호 01086046995
- ✅ Lightsail 방화벽 포트 3001 오픈
- ✅ 실제 SMS 전송 테스트 완료
# Force rebuild Sat Feb  7 13:37:07 UTC 2026
# Deploy Sat Feb  7 22:59:47 UTC 2026
Last updated: Sun Feb  8 00:46:30 UTC 2026


# D1 bindings configured for both Production and Preview

# Sessions table created: Sun Feb  8 10:25:52 UTC 2026

# Testing D1 session storage - deployment trigger: Sun Feb  8 13:25:00 UTC 2026
